...
export default function Contracts() {
  ...
  const reload = async (targetPage = page) => {
    ...
    const { contracts: data, pagination: p } = await getContracts({
      companyId: selectedCompanyId, status: statusFilter || undefined, page: targetPage, limit: PAGE_SIZE,
    });
    setContracts(data);
    setPagination(p);
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setError("");
        const { contracts: data, pagination: p } = await getContracts({
          companyId: selectedCompanyId, status: statusFilter || undefined, page, limit: PAGE_SIZE,
        });
        if (cancelled) return;
        setContracts(data);
        setPagination(p);
