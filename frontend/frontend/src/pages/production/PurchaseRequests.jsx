...
export default function PurchaseRequests() {
  ...
  // ---------- Translations editor (notes) ----------
  const [translatingRequest, setTranslatingRequest] = useState(null);
  const handleTranslationSaved = (field, bucket) => {
    setRequests((prev) =>
      prev.map((r) =>
        r._id === translatingRequest?._id
          ? { ...r, translations: { ...r.translations, [field]: bucket } }
          : r
      )
    );
  };
  ...
  return (
    <div className="pageShell">
            ...
            {requests.map((request) => (
              <div key={request._id} className="dataTableRow" style={{ gridTemplateColumns: gridColumns }}>
                ...
                <div className="dataTableActions">
                  ...
                  <button type="button" className="tableActionBtn" title={t("contentTranslation.editButton")} onClick={() => setTranslatingRequest(request)}>
                    <Languages size={14} />
                  </button>
                </div>
              </div>
      ...
      <TranslationEditorModal
        isOpen={!!translatingRequest}
        onClose={() => setTranslatingRequest(null)}
        resourceType="purchaseRequest"
        resourceId={translatingRequest?._id}
        fields={[{ key: "notes", label: t("salaries.fields.notes") }]}
