...
import { Clock, BriefcaseBusiness, Languages } from "lucide-react";
...
export default function Attendance() {
  ...
  const [translatingRecord, setTranslatingRecord] = useState(null);
  const handleTranslationSaved = (field, bucket) => {
    setRecords((prev) =>
      prev.map((r) =>
        r._id === translatingRecord?._id
  ...
  return (
    <div className="pageShell">
            ...
            {records.map((r) => (
              <div key={r._id} className="dataTableRow" style={{ gridTemplateColumns: gridColumns }}>
                ...
                <button
                  ...
                  onClick={() => setTranslatingRecord(r)}
                >
                  <Languages size={14} />
                </button>
              </div>
      ...
      <TranslationEditorModal
        isOpen={!!translatingRecord}
        onClose={() => setTranslatingRecord(null)}
        resourceType="attendance"
        resourceId={translatingRecord?._id}
        fields={[{ key: "notes", label: t("attendance.fields.notes") }]}
