 ...
 */
// Returns a PLAIN object. Callers copy it with { ...config }, and
// spreading a Mongoose subdocument copies its internals ($__, _doc),
// not its fields — every schedule then silently resolved to the
// defaults (09:00-17:00, continuous) whatever the company configured.
workScheduleSchema.methods.getDayConfig = function getDayConfig(date) {
  const key = DAY_KEYS[date.getDay()];
  const day = this[key];
  return day && typeof day.toObject === "function" ? day.toObject() : day;
};
