...
export default function Employees() {
  ...
  // ---------- Translated display values for the selected employee ----------
  // Called unconditionally (rules of hooks) — useTranslatedField is
  // null-safe, so this is harmless while selectedEmployee is null.
  const translatedJobTitle = useTranslatedField(selectedEmployee, "jobTitle");
  const translatedService = useTranslatedField(selectedEmployee, "service");
  const translatedPosition = useTranslatedField(selectedEmployee, "position");
  ...
  const handleTranslationSaved = (field, bucket) => {
    const patch = (emp) =>
      emp._id === translatingEmployee?._id
        ? { ...emp, translations: { ...emp.translations, [field]: bucket } }
        : emp;
  ...
  return (
    <div className="pageShell">
      ...
      <TranslationEditorModal
        isOpen={!!translatingEmployee}
        onClose={() => setTranslatingEmployee(null)}
        resourceType="employee"
        resourceId={translatingEmployee?._id}
        fields={[
          { key: "jobTitle", label: t("employees.fields.jobTitle") },
          { key: "service", label: t("employees.fields.service") },
          { key: "position", label: t("employees.fields.position") },
          { key: "workLocation", label: t("employees.fields.workLocation") },
